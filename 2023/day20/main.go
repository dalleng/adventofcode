...
func main() {
	...
	// Step 0. Reset flipFlopState and conjunctionState
	for module := range flipFlopState {
		flipFlopState[module] = false
	}
	for _, inputs := range conjunctionState {
		for module := range inputs {
			inputs[module] = LOW
		}
	}
	...
	// Step 3.
	// Count the button presses needed for each module that
	// that is an input to `conjunctionToRx` to be HIGH
	for {
	...
	// Step 4.
	// Calculate the least common multiple, so you get the amount
	// of button presses needed for all of the inputs to `conjunctionToRx`
	// to be HIGH
...
// Function used for the solution of the 2nd part
